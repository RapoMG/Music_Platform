...
class UserPublicPlaylistListView(ListAPIView):
    ...
    def get_queryset(self) -> Playlist:
        ...
        return Playlist.objects.filter(user=user, is_public=True).select_related('user')
    

class PublicPlaylistDetailView(RetrieveAPIView):
    ...
    def get_object(self):
        ...
        raise PermissionDenied("Playlist is not public")

# Private

class PlaylistViewSet(ModelViewSet):
    """CRUD for user's playlists."""
    serializer_class = PlaylistSerializer
...
class PlaylistItemViewSet(GenericViewSet):
    """CRUD for playlist items. """
    
    permission_classes = [IsAuthenticated]

    def get_playlist(self, request, playlist_id) -> Playlist:
        """Get a playlist by ID."""
        playlist = get_object_or_404(Playlist, id=playlist_id)

        # Check if the playlist belongs to the user
        if playlist.user != request.user:
            raise PermissionDenied("Not your playlist")
    ...
    def add_song(self, request, playlist_id=None) -> Response:
        ...
        try:
            add_song_to_playlist(request.user, song, playlist)
